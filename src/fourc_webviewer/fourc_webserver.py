...
class FourCWebServer:
    ...
    def init_pyvista_render_objects(self):
        ...
        geometry_file_names = get_geometry_file(self._server_vars["fourc_yaml_content"])

        if geometry_file_names:
            # ensure that geometry_file_name really only contains the name and not a path
            geometry_file_name = Path(geometry_file_names[0]).name
            if not (
    ...
    def request_included_files(self):
        ...
        geometry_file_names = get_geometry_file(self._server_vars["fourc_yaml_content"])
        self.append_include_files(geometry_file_names)
        # add yaml includes
        yaml_include_names = [
            Path(file_path).name
